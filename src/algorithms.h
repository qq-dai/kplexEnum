...
public:
    ...
    void testprintGraph();
    int core_decompsition(vector<int> &nodeset, int nodesize);
	int coloring(vector<int> &nodeset, int nodesize);
...
typedef struct _Node
{
	...
	void cinsert(const int &x, const int &c) {
		nodes.emplace_back(x,c);
		size++;
	}
	void einsert(const int &x, const int &c) {
		nodes.emplace_back(x, c);
		size++;
		exsize++;
	}
	void cinsert(const int &id, const int &x, const int &c) {
		nodes[id].first  = x;
		nodes[id].second = c;
		size++;
	}
	void einsert(const int &id, const int &x, const int &c) {
		nodes[id].first  = x;
		nodes[id].second = c;
		size++;
		exsize++;
